...
<PackageInstall packageName="@srinivas-narayanam/ui" />
...
<PackageInstall packageName="@srinivas-narayanam/ui-preset" devDependency />
...
```tsx
module.exports = {
  presets: [require("@srinivas-narayanam/ui-preset")],
  // ...
}
```

In order for the styles to be applied correctly to the components, you will also need to ensure that
`@srinivas-narayanam/ui` is included in the content field of your `tailwind.config.js` file:

```tsx
module.exports = {
  content: [
    // ...
    "./node_modules/@srinivas-narayanam/ui/dist/**/*.{js,jsx,ts,tsx}",
  ],
  // ...
}
```

If you are working within a monorepo, you may need to add the path to the `@srinivas-narayanam/ui` package in your `tailwind.config.js` like so:

```tsx
const path = require("path")

const uiPath = path.resolve(
  require.resolve("@srinivas-narayanam/ui"),
  "../..",
  "\*_/_.{js,jsx,ts,tsx}"
)
...
```tsx
import { Button, Drawer } from "@srinivas-narayanam/ui"
```
