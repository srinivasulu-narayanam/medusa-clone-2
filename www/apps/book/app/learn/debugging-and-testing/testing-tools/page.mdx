...
## @srinivas-narayanam/test-utils Package
...
To use the Testing Framework, install `@srinivas-narayanam/test-utils` as a `devDependency`:

```bash npm2yarn
npm install --save-dev @srinivas-narayanam/test-utils@latest
```
...
Writing tests with `@srinivas-narayanam/test-utils`'s tools requires installing and configuring Jest in your project.
...
const { loadEnv } = require("@srinivas-narayanam/framework/utils")
...
The next chapters explain how to use the testing tools provided by `@srinivas-narayanam/test-utils` to write tests.
