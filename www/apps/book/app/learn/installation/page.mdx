...
```bash
npx medusa user -e admin@srinivas-narayanam.com -p supersecret
```

Replace `admin@srinivas-narayanam.com` and `supersecret` with the user's email and password respectively.
...
To update your Medusa application, bump the version of all `@srinivas-narayanam/*` dependencies in your `package.json`. Then, re-install dependencies:
