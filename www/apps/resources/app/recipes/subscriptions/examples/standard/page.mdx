...
import CartModule from "@srinivas-narayanam/medusa/cart"
...
import { defineLink } from "@srinivas-narayanam/framework/utils"
import SubscriptionModule from "../modules/subscription"
import CustomerModule from "@srinivas-narayanam/medusa/customer"
...
```ts title="src/modules/subscription/types/index.ts"
import { InferTypeOf } from "@srinivas-narayanam/framework/types"
import Subscription from "../models/subscription"
...
Medusa provides the first and last steps in the `@srinivas-narayanam/medusa/core-flows` package, so you only need to implement the second step.
...
import { LinkDefinition } from "@srinivas-narayanam/framework/types"
...
1. `completeCartWorkflow` from `@srinivas-narayanam/medusa/core-flows` that completes a cart and creates an order.
2. `useQueryGraphStep` from `@srinivas-narayanam/medusa/core-flows` to retrieve the order's details.
3. `createSubscriptionStep`, which is the step you created previously.
4. `createRemoteLinkStep` from `@srinivas-narayanam/medusa/core-flows`, which accepts links to create. These links are in the `linkDefs` array returned by the previous step.
...
import { StoreCart } from "@srinivas-narayanam/framework/types"
import { Button, clx, Heading, Text } from "@srinivas-narayanam/ui"
import { CheckCircleSolid } from "@srinivas-narayanam/icons"
...
import { ClockSolid } from "@srinivas-narayanam/icons"
import { Container, Heading, Table, Badge } from "@srinivas-narayanam/ui"
...
1. Retrieve the subscription’s linked cart. Medusa provides a `useQueryGraphStep` in the `@srinivas-narayanam/medusa/core-flows` package that can be used as a step.
2. Create a payment collection for the new order. Medusa provides a `createPaymentCollectionsStep` in the `@srinivas-narayanam/medusa/core-flows` package that you can use.
3. Create payment sessions in the payment collection. Medusa provides a `createPaymentSessionsWorkflow` in the `@srinivas-narayanam/medusa/core-flows` package that can be used as a step.
4. Authorize the payment session. Medusa also provides the `authorizePaymentSessionStep` in the `@srinivas-narayanam/medusa/core-flows` package, which can be used.
5. Create the subscription’s new order.
6. Create links between the subscription and the order using the `createRemoteLinkStep` provided in the `@srinivas-narayanam/medusa/core-flows` package.
7. Capture the order’s payment using the `capturePaymentStep` provided by Medusa in the `@srinivas-narayanam/medusa/core-flows` package.
...
import { createOrdersWorkflow } from "@srinivas-narayanam/medusa/core-flows"
...
This creates a `createSubscriptionOrderStep` that uses the `createOrdersWorkflow`, which Medusa provides in the `@srinivas-narayanam/medusa/core-flows` package. The step returns the created order and an array of links to be created.
...
import { MedusaContainer } from "@srinivas-narayanam/framework/types"
import SubscriptionModuleService from "../modules/subscription/service"
import { SUBSCRIPTION_MODULE } from "../modules/subscription"
import moment from "moment"
