...
2. `setAuthAppMetadataStep` to create the `vendor` actor type. This step is provided by Medusa in the `@srinivas-narayanam/medusa/core-flows` package.
...
import { AuthenticatedMedusaRequest, MedusaResponse } from "@srinivas-narayanam/framework/http"
import { 
...
// other imports...
import { createProductsWorkflow } from "@srinivas-narayanam/medusa/core-flows"
import { 
...
1. Retrieve the cart using its ID. Medusa provides a `useQueryGraphStep` in the `@srinivas-narayanam/medusa/core-flows` package that you can use.
2. Create a parent order for the cart and its items. Medusa also has a `completeCartWorkflow` in the `@srinivas-narayanam/medusa/core-flows` package that you can use as a step.
3. Group the cart items by their product’s associated vendor.
4. Retrieve the order's details using Medusa's `getOrderDetailWorkflow` exported by the `@srinivas-narayanam/medusa/core-flows` package.
...
6. Create the links created by the previous step. Medusa provides a `createRemoteLinkStep` in the `@srinivas-narayanam/medusa/core-flows` package that you can use.
...
import { CartDTO, CartLineItemDTO } from "@srinivas-narayanam/framework/types"
...
import { Modules, promiseAll } from "@srinivas-narayanam/framework/utils"
import { 
  cancelOrderWorkflow,
  createOrderWorkflow,
} from "@srinivas-narayanam/medusa/core-flows"
...
If an error occurs, the created orders in the `createdOrders` array are canceled using Medusa's `cancelOrderWorkflow` from the `@srinivas-narayanam/medusa/core-flows` package.
...
The compensation function cancels all child orders received from the step. It uses the `cancelOrderWorkflow` that Medusa provides in the `@srinivas-narayanam/medusa/core-flows` package.
...
import { AuthenticatedMedusaRequest, MedusaResponse } from "@srinivas-narayanam/framework/http"
import { ContainerRegistrationKeys } from "@srinivas-narayanam/framework/utils"
import { getOrdersListWorkflow } from "@srinivas-narayanam/medusa/core-flows"
import MarketplaceModuleService from "../../../modules/marketplace/service"
import { MARKETPLACE_MODULE } from "../../../modules/marketplace"
