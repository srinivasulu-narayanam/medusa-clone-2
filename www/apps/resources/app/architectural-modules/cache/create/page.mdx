...
The Cache Module's main service must implement the `ICacheService` interface imported from `@srinivas-narayanam/framework/types`:

```ts title="src/modules/my-cache/service.ts"
import { ICacheService } from "@srinivas-narayanam/framework/types"
