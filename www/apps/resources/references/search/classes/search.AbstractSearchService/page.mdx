...
A search service must extend the `AbstractSearchService` class imported from the `@srinivas-narayanam/framework/utils` package. If you don’t already have the package 
installed, run the following command to install it within your project:

```bash npm2yarn
npm install @srinivas-narayanam/utils
```
...
A search service class is in a TypeScript or JavaScript file created in the `src/services` directory. The class must extend the `AbstractSearchService` class imported
 from the `@srinivas-narayanam/utils` package.
...
import { AbstractSearchService } from "@srinivas-narayanam/utils"
...
```ts
// ...
import { ProductService } from "@srinivas-narayanam/medusa"
