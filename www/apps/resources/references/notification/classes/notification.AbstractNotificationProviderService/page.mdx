...
The Notification Provider Module's main service must extend the `AbstractNotificationProviderService` class imported from `@srinivas-narayanam/framework/utils`:
...
```ts
import { AbstractNotificationProviderService } from "@srinivas-narayanam/framework/utils"
