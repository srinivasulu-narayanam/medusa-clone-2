...
Create the file `src/modules/my-file/service.ts` that holds the implementation of the module's main service. It must extend the `AbstractFileProviderService` class imported from `@srinivas-narayanam/framework/utils`:

```ts title="src/modules/my-file/service.ts"
import { AbstractFileProviderService } from "@srinivas-narayanam/framework/utils"
...
module.exports = defineConfig({
  ...
  modules: [
    {
      resolve: "@srinivas-narayanam/medusa/file",
      options: {
        providers: [
          // default provider
          {
            resolve: "@srinivas-narayanam/medusa/file-local",
            id: "local",
          },
          {
